...
#强制移除kmod-nf-ipt6避免ip6_tables.ko缺失错误
KMOD_IPT6=$(find ../feeds -path "*/kmod-nf-ipt6/Makefile" 2>/dev/null)
if [ -n "$KMOD_IPT6" ]; then
	mv "$KMOD_IPT6" "$KMOD_IPT6.bak"
	echo "kmod-nf-ipt6 has been removed!"
fi
